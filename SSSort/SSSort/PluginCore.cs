...
namespace SSSort
{
	...
	public class PluginCore : PluginBase
	{
		...
		private static readonly RuleInfo[] SalvageRules = {
			...
			new RuleInfo("Material", "Alphabetical", "Reverse Alpha", true, true),
...
#pragma warning disable 649
		[ControlReference("lblSortItemsIn")]
		private StaticWrapper lblSortItemsIn;

		[ControlReference("btnSort")]
		private PushButtonWrapper btnSort;

		[ControlReference("nbkMode")]
		private NotebookWrapper nbkMode;
		...
		[ControlReference("chkSortToBottom")]
		private CheckBoxWrapper chkSortToBottom;

		[ControlReference("prgProgress")]
		private ProgressWrapper prgProgress;

		[ControlReference("lstScroll")]
		private ListWrapper lstScroll;

		[ControlReference("lstSalvage")]
		private ListWrapper lstSalvage;

		[ControlReference("lstManaStones")]
		private ListWrapper lstManaStones;
		...
		[ControlReference("lblPluginNameVer")]
		private StaticWrapper lblPluginNameVer;

		[ControlReferenceArray("btnMainPack",
			"btnPack1", "btnPack2", "btnPack3", "btnPack4",
			"btnPack5", "btnPack6", "btnPack7", "btnPack8")]
		private ButtonWrapper[] packButtons;
#pragma warning restore 649
		...
		protected override void Startup()
		{
			try
			{
				...
				mMainViewDefaultHeight = DefaultView.Position.Height;
			}
		...
		{
			Rectangle rect = DefaultView.Position;
			rect.Height = height;
			SetViewRegion(rect);
		}
			...
			public int Compare(WorldObject x, WorldObject y)
			{
				...
				for (int r = 0; r < parent.lstSalvage.RowCount; r++)
				{
					if ((bool)parent.lstSalvage[r][PriorityList.Enabled][0])
					{
						...
						{
							string xMaterial = x.Name;
							int paren = xMaterial.IndexOf('(');
							if (paren > 0)
								xMaterial = xMaterial.Substring(0, paren).Trim();

							string yMaterial = y.Name;
							paren = yMaterial.IndexOf('(');
							if (paren > 0)
								yMaterial = yMaterial.Substring(0, paren).Trim();

							retVal = StringComparer.Ordinal.Compare(xMaterial, yMaterial);
							done = (retVal != 0);
						}
		...
		{
			ILayer btn = (ILayer)btnSort.Underlying;
			tagRECT pos = btn.get_Position();
			if (mSortState != SortState.Idle)
			{
				btnSort.Text = "Stop Sorting";
				pos.right = pos.left + 224;
			}
			else
			{
				btnSort.Text = "Selected Pack";
				pos.right = pos.left + 108;
			}
			btn.set_Position(ref pos);
		...
		[ControlEvent("nbkMode", "Change")]
		private void nbkMode_Change(object sender, IndexChangeEventArgs e)
		{
			try
			{
				UpdateSortButtonText();
		...
		[ControlEvent("btnSort", "Click")]
		private void btnSort_Click(object sender, ControlEventArgs e)
		{
			try
			{
				if (mSortState != SortState.Idle)
		...
		[ControlEvent("lstScroll", "Selected")]
		private void lstScroll_Selected(object sender, ListSelectEventArgs e)
		{
			try
			{
		...
		[ControlEvent("lstSalvage", "Selected")]
		private void lstSalvage_Selected(object sender, ListSelectEventArgs e)
		{
			try
			{
		...
		[ControlEvent("lstManaStones", "Selected")]
		private void lstManaStones_Selected(object sender, ListSelectEventArgs e)
		{
			try
			{
		...
		private void HandleListClick(ListWrapper lst, int row, int col)
		{
			switch (col)
			{
		...
		[ControlEvent("btnMainPack", "Click")]
		[ControlEvent("btnPack1", "Click")]
		[ControlEvent("btnPack2", "Click")]
		[ControlEvent("btnPack3", "Click")]
		[ControlEvent("btnPack4", "Click")]
		[ControlEvent("btnPack5", "Click")]
		[ControlEvent("btnPack6", "Click")]
		[ControlEvent("btnPack7", "Click")]
		[ControlEvent("btnPack8", "Click")]
		private void btnMainPack_Click(object sender, ControlEventArgs e)
		{
			try
			{
		...
		[ControlEvent("btnAllPacks", "Click")]
		private void btnAllPacks_Click(object sender, ControlEventArgs e)
		{
			try
			{
		...
		#region Settings
		private void AddPriority(ListWrapper list, RuleInfo info, bool ascending, bool enabled, int choiceCount)
		{
		...
		private void LoadSettings()
		{
			try
			{
				FileInfo settingsFile = new FileInfo(Util.FullPath("settings.xml"));
				...
				foreach (XmlElement ele in doc.DocumentElement.SelectNodes("option"))
				{
					...
					switch (ele.GetAttribute("name"))
					{
						case "ViewPosition":
							...
							{
								DefaultView.Position = new Rectangle(pointVal, DefaultView.Position.Size);
							}
							break;
